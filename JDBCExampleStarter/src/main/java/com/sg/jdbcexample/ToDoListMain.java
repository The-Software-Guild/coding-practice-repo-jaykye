...
public class ToDoListMain {
    ...
    private static void displayList() throws SQLException {
        try ( Connection conn = ds.getConnection()) {  // 괄호 안에 있는게 python에서 with 이랑 같음.
            Statement stmt = conn.createStatement();  // edit 할 때는 PreparedStatement를 사용해라.
    ...
    private static void addItem() throws SQLException {
        ...
        String task = sc.nextLine();  // 이게 훨씬 간단하네...
        System.out.println("Any additional notes?");
        String note = sc.nextLine();

        try (Connection conn = ds.getConnection()) {
            String sql = "INSERT INTO todo(todo, note) VALUES(?,?)";
            PreparedStatement pStmt = conn.prepareCall(sql); // 이게 injection attack을 방지해줌.
            // 아마 INSERT into tableName values(1, 2..) 이런 식으로 가는 듯.
            pStmt.setString(1, task);  // 위의 튜플 인덱스 대로.
    ...
    private static void removeItem() throws SQLException {
        ...
        try(Connection conn = ds.getConnection()) {
            ...
            PreparedStatement pStmt = conn.prepareCall(sql);
            pStmt.setString(1, itemId);  // Null 값 대입하는 것. delete와 같다.
            pStmt.executeUpdate();
